  ...
  return (
    <div className="container mx-auto pt-10 px-4">
      ...
      <p className="text-center mb-8 text-gray-500">
        Explore available NFT collections on the marketplace.
      </p>
      ...
      {listingsLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {Object.entries(collections).map(([contractAddress, listings]) => {
            ...
            const floorPrice = hasListings ? listings.reduce((min: any, l: any) => (l.price < min ? l.price : min), listings[0].price) : 0n;
            const volume = hasListings ? listings.reduce((sum: any, l: any) => sum + l.price, 0n) : 0n;
            
            const isYourCollectible = yourCollectibleInfo && contractAddress.toLowerCase() === yourCollectibleInfo.address.toLowerCase();
            const totalItems = isYourCollectible && totalSupply ? totalSupply.toString() : "N/A";

            return (
              <div key={contractAddress} className="card bg-base-100 shadow-xl border border-base-300 hover:shadow-2xl transition-all duration-200">
